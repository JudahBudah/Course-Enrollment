...
function renderCalendar() {
    ...
    for (let d = 1; d <= (trailing === 0 ? 0 : 7 - trailing); d++) addCell(d, true);
}
...
function onCalDayClick(day, e) {
    ...
    if (evs.length > 0 && peek && peekText) {
        peekText.innerHTML = evs.map(ev =>
            `<span onclick="openCalEventModal(window._calEvents.find(x=>x.event_id=='${ev.event_id}'))" `+
            ...
            `<span style="text-decoration:underline;">${escHtml(ev.title)}</span></span>`
        ).join('');
...
function openCalEventModal(ev) {
    ...
    document.getElementById('cem_banner_wrap').innerHTML = ev.image
        ...
        : '';
    document.getElementById('cem_color_strip').style.background = ev.color || '#8C1C24';
    ...
    if (ev.event_time) meta += `<span style="display:flex;align-items:center;gap:4px;"><i class="fa-solid fa-clock" style="color:#8C1C24;"></i> ${ev.event_time}</span>`;
...
function showCalendarDay(dayName, date) {
    ...
    if (classes.length === 0) {
        ...
        listToday.innerHTML = `<div class="sched-day-label" style="border-top:none;">${dateStr}</div>` +
            classes.map(s => `
                <div class="schedule-item">
                    <div class="schedule-time">${escHtml(s.schedule_time || 'TBA')}</div>
                    <div class="schedule-details">
                        <h4>${escHtml(s.subject_name)}</h4>
                        <p>${escHtml(s.room || 'TBA')} &bull; ${escHtml(s.faculty_name || 'TBA')}</p>
                    </div>
                </div>`).join('');
    }
}
...
function setRange(range) {
    ...
    document.getElementById('btnToday').classList.toggle('active', range === 'today');
    document.getElementById('btnWeek').classList.toggle('active',  range === 'week');
    document.getElementById('listToday').style.display = range === 'today' ? '' : 'none';
    document.getElementById('listWeek').style.display  = range === 'week'  ? '' : 'none';
...
function setMode(mode) {
    ...
    document.getElementById('schedListView').style.display = mode === 'list' ? '' : 'none';
    document.getElementById('schedGridView').style.display = mode === 'grid' ? '' : 'none';
...
function renderMiniGrid() {
    ...
    Object.entries(weekSchedule).forEach(([dayName, classes]) => {
        ...
        classes.forEach(cls => {
            ...
            let entry = scheduleData.find(e => e.code === cls.subject_code && e.start === start);
            if (entry) {
                if (!entry.slots.find(sl => sl.day === dayKey)) entry.slots.push({day: dayKey, start, end});
            } else {
    ...
    const PX = 0.75;
    const allStarts = scheduleData.flatMap(s => s.slots.map(sl => toMin(sl.start) / 60 | 0));
    const allEnds   = scheduleData.flatMap(s => s.slots.map(sl => Math.ceil(toMin(sl.end) / 60)));
    ...
    const ALL_LABELS = {M:'MON',T:'TUE',W:'WED',TH:'THU',F:'FRI',S:'SAT'};
    const DAYS   = allowedDays ? ALL_DAYS.filter(d => allowedDays.has(d)) : ALL_DAYS;
    ...
    container.style.cssText = `display:grid;grid-template-columns:36px repeat(${colCount},1fr);background:var(--white-pr);min-width:${colCount > 1 ? 420 : 160}px;padding-bottom:12px;`;

    const tcol = mk('div','wg-time-col');
    tcol.appendChild(mk('div','wg-time-header'));
    const tb = mk('div'); tb.style.cssText = `position:relative;height:${GRID_H}px;`;
    ...
    DAYS.forEach(day => {
        ...
        const body = mk('div','wg-body-area'); body.style.height = `${GRID_H}px`;

        for (let h = S_HOUR; h < E_HOUR; h++) {
            const min = (h-S_HOUR)*60;
            const hl = mk('div','wg-hour-line'); hl.style.top=`${min*PX}px`; body.appendChild(hl);
            const hf = mk('div','wg-half-line'); hf.style.top=`${(min+30)*PX}px`; body.appendChild(hf);
        }

        scheduleData.forEach(subj => {
            subj.slots.forEach(slot => {
                if (slot.day !== day) return;
                const top = toTop(slot.start), hgt = toHgt(slot.start, slot.end);
                ...
                if (hgt > 28) block.appendChild(mk('div','wg-block-name', subj.shortName));
